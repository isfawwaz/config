...
{
  const maximumLength =
    'We are writting this to check which one is the maximum length of our program. It should be somewhere around here-[ x ]';
}
// Good
{
...
{
  const a =
    Math.random() > 0.5 ? true : Math.random() > 0.3 ? true : Math.random() > 0.5 ? 'foo' : 'bar';
}
// Good
{
